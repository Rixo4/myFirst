  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLoginView) {
        ...
        // Supabase trigger or frontend logic would normally create the profile here,
        // but our table schema creates the row automatically or we can insert it.
        if (data?.user) {
          const { error: profileError } = await supabase
            .from('profiles')
            .insert([{ id: data.user.id }]);
            
          // If inserting the profile fails, we might just log it and proceed
